...
describe('Multiline comments support', () => {
	...
	it(
		String.raw`should convert \n in comments to actual multiline descriptions in YAML`,
		async () => {
		...
		`;
			...
			expect(
				(userSchema.properties!.firstName as SchemaObject).description,
			).toBe("The user's\nfirst name");
	...
	it(
		String.raw`should convert \n in comments to actual multiline descriptions in JSDoc`,
		async () => {
			// Define schema inline with field comments containing \n
			const schema = `
			datasource db {
